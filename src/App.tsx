...
function App() {
  ...
  const handleFilterChange = (event: ChangeEvent) => {
    setActiveFilter(((event.target as HTMLSelectElement).value) as ActiveFilterType)
  }

  const filteredAirports: Airport[] = activeFilter === "all"
    ? airports
    : airports.filter((airport) => airport.active === (activeFilter === "true"))
